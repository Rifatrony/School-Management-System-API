...
const fetchAllSection = async (req, res) => {
    try {
        const sections = await Section.find({ class_id: req.params.class_id }).select("id name class_id department_id");

        const sectionsWithDetails = await Promise.all(sections.map(async section => {
            // Fetch class details
            const classDetails = await Class.findOne({ id: section.class_id }).select({ _id: 0, id: 1, name: 1 });
